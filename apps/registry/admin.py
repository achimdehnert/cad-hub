...
class ModulePricingInline(admin.TabularInline):
    model = ModulePricing
    extra = 1
    fields = ["organization", "pricing_type", "setup_eur", "monthly_eur", "label", "valid_from", "valid_until"]
...
class NL2CADModuleAdmin(admin.ModelAdmin):
    list_display  = [
        "id", "name", "icon_display", "package", "status_badge",
        "priority", "story_points", "target_quarter", "standard_price_display",
        "subscription_count",
    ]
    ...
    def icon_display(self, obj: NL2CADModule) -> str:
        return format_html(
            '<span style="font-size:1.4em;">{}</span>', obj.icon
        )
...
class ModulePricingAdmin(admin.ModelAdmin):
    list_display  = ["module", "organization", "pricing_type", "setup_eur", "monthly_eur", "label", "valid_from", "valid_until"]
...
class TenantSubscriptionAdmin(admin.ModelAdmin):
    list_display  = [
        "organization", "module", "status_badge", "monthly_eur",
        "berufsprofil", "approved_by", "activated_at", "created_at",
    ]
