...
class DXFRendererService:
    ...
    def export_to_json(self) -> dict | None:
        """
    ...
    def get_thumbnail_svg(self, max_size: int = 200) -> str | None:
        """Generate a small SVG thumbnail."""
        return self.render_to_svg(width=max_size, height=max_size)

    def export_for_threejs(self) -> dict | None:
        """
...
def render_dxf_to_svg(filepath: str | Path, output: Path | None = None) -> str | None:
...
def dxf_to_json(filepath: str | Path) -> dict | None:
