...
class SubdomainTenantMiddleware(MiddlewareMixin):
    ...
    def _user_may_access(request: HttpRequest, org: Organization) -> bool:
        ...
        Threat model: an *authenticated* user must not enter a foreign
        organization's context (and read its data) merely by visiting
        ``acme.<host>`` or sending ``X-Tenant-ID: <uuid>``. Anonymous
        requests are allowed through (public tenant pages, no user-scoped
        data to leak), keeping the change backward-compatible.
        """
        user = getattr(request, "user", None)
